...
static void make_device(char *path, int delete)
{
	...
	device_name = bb_basename(path);   /* 获取设备文件的名字 path=/sys/DEVPATH，DEVPATH=class/..../dev*/
	type = path[5]=='c' ? S_IFCHR : S_IFBLK;  /* 获取设备的类型，是字符设备还是块设备等 (path=/sys/class/....  ,path[5]='c'--->表明是字符设备)*/
	...
	if (ENABLE_FEATURE_MDEV_CONF) {
		...
		fd = open("/etc/mdev.conf", O_RDONLY);   /* 如果有配置文件的话 */
		if (fd < 0)
			goto end_parse;
 ...
 end_parse:	/* nothing */ ;
	...
	umask(0);
	if (!delete) { 
		if (sscanf(temp, "%d:%d", &major, &minor) != 2) return;   /* 读取/sys/class/..../dev内容确定 主次设备号*/
		if (mknod(device_name, mode | type, makedev(major, minor)) && errno != EEXIST)  /* 创建设备节点 */
	...
	if (delete) unlink(device_name);   /* 如果是删除设备的话则调用unlink系统调用来删除设备文件 */
}
...
int mdev_main(int argc, char **argv)
{
	...
	if (argc == 2 && !strcmp(argv[1],"-s")) {  /* /etc/init.d/rcS脚本中有一句脚本为mdev -s   意思是在引导时扫描/sys目录，然后参数/dev目录 */
		struct stat st;
	...
	/* Hotplug */
		...
		action = getenv("ACTION");   /* 获得环境变量ACTION,ACTION=add 或remove*/
		env_path = getenv("DEVPATH");  /* 获取环境变量:路径 */
		if (!action || !env_path)
			bb_show_usage();

		sprintf(temp, "/sys%s", env_path);    /* temp=/sys/ DEVPATH*/
		if (!strcmp(action, "remove"))   /* 如果 action=remove，则删除设备*/
			make_device(temp, 1);
		else if (!strcmp(action, "add")) {   /*如果action=add则创建设备*/
			make_device(temp, 0);
